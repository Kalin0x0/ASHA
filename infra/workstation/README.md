...
Builds Chista session images that **trust the deployment's internal CA** and are
...
- Sets `CHISTA_DLP_ENABLED` (honoured by DLP-capable KasmVNC builds).
...
## Build
```bash
./build.sh                                   # → chista/firefox-trusted:1.16.0
./build.sh kasmweb/chrome:1.16.0             # → chista/chrome-trusted:1.16.0
./build.sh kasmweb/firefox:1.16.0 myimg:tag 1
```

## Use
Set a Workspace's image (Admin → Workspaces → Images, or the registry install
`imageOverride`) to the built tag, e.g. `chista/firefox-trusted:1.16.0`. New
