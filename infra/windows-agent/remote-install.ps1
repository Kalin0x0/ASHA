<#
.SYNOPSIS
  Remotely install the Asha host agent on one or more Windows desktops/servers
...
.DESCRIPTION
  ...
  so the host auto-registers with Asha and reports availability.
  ...
  manages; for Asha-provisioned VMs, bake the agent into the golden template.)
...
$here = $PSScriptRoot
$agentSrc = Join-Path $here 'asha-agent.ps1'
$installSrc = Join-Path $here 'install.ps1'
if (-not (Test-Path $agentSrc) -or -not (Test-Path $installSrc)) {
  throw "asha-agent.ps1 / install.ps1 not found next to this script ($here)."
}
...
foreach ($target in $ComputerName) {
  ...
  try {
    ...
    $remoteDir = 'C:\ProgramData\Asha'
    Invoke-Command -Session $session -ScriptBlock {
      param($d) New-Item -ItemType Directory -Force -Path $d | Out-Null
    } -ArgumentList $remoteDir

    Copy-Item -ToSession $session -Path $agentSrc   -Destination "$remoteDir\asha-agent.ps1" -Force
    Copy-Item -ToSession $session -Path $installSrc -Destination "$remoteDir\install.ps1"      -Force

    Invoke-Command -Session $session -ScriptBlock {
      param($d, $url, $tok, $rdp)
      $a = @{ AshaUrl = $url; Token = $tok }
      if ($rdp) { $a['EnableRdp'] = $true }
      & "$d\install.ps1" @a
    } -ArgumentList $remoteDir, $AshaUrl, $Token, [bool]$EnableRdp
